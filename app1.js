...
// Initialize the page
//FUCK CSS 
//Custom code added by 37
/* 
  //Put me in authConfig
  //37CUSTOM
  "login_using_questionmark" : true, //Login using question mark example: https://example.com/0:/Folder/Video.mkv?username:password or if you have ?a=view in it then https://example.com/0:/Folder/Video.mkv?a=view?username:password
*/
/* 
  //Put me in uiConfig
  //37CUSTOM
  "enable_custom_script": true, // Enable custom script on file list
  "custom_script": "", // Custom script that will add in file list
  "display_video_drive_link": false, // This will add GD Video Link button.
  "request_episodes_list": true, // Create iframe to to get list of video files work also on phone but sometimes just doesn't work
  "show_select_episode_by_request": true, // Set this to true to show Select episode (doesn't work without request_episodes_list)
  "show_next_and_previous_buttons": true, //Set this to true to show next and previous_buttons (doesn't work without request_episodes_list)
  "disable_gdrive_player": false, // Set this to true to hide gdrive player
  "disable_rotate_when_fullscreen": false, // Disable this option to disable screen rotation to when it is in full screen
  "hide_full_url": true, // Hide Full Url element in ?a=view
  "change_number_of_disc_to_name": true, // Change numer of disc example 1: to Name of disk
  "show_only_video_files": true, // Show only video files in file list and search results
  "display_drive_link": true, //Bring back GDrive Link button
  "detect_site_as_webapp": true, //Detect site as webapp
  "disable_darkreader": true, //Disable darkreader on website
  "show_credentials_in_url": true, //Show credentials in url so you can example play video files in mpv or VLC
*/
function init() {
    ...
    if (!model.is_search_page) {
        ...
        if (arr.length > 1) {
            for (var i in arr) {
                ...
                //kacperhaha's idea
                if (UI.change_number_of_disc_to_name && n === `${window.current_drive_order}:`) {
                    n = window.drive_names[window.current_drive_order || 0];
                }
...
function list(path, id = '', fallback = false) {
    var containerContent = `<script>${UI.enable_custom_script ? UI.custom_script: ''}</script>`
    //console.log(id);//USELESS
    containerContent += `<div class="container">${UI.fixed_header ?'<br>': ''}
    <div id="update"></div>
    ...
    if (navarray.length > 1) {
        for (var i in navarray) {
            ...
            //kacperhaha's idea
            if (displayedPathPart === `${window.current_drive_order}:`) {
                displayedPathPart = window.drive_names[window.current_drive_order || 0];
            }
  ...
  <div id="readme_md" style="display:none; padding: 20px 20px;"></div>
    ...
    function handleSuccessResult(res, path, prevReqParams) {
        //console.log(res, path, prevReqParams);//USELESS
        $('#list')
    ...
    try {
        ...
        for (i in files) {
            ...
            if (item['mimeType'] == 'application/vnd.google-apps.folder') {
                ...
                //kacperhaha's idea
                if (UI.show_only_video_files && !["mp4", "webm", "avi", "mpg", "mpeg", "mkv", "rm", "rmvb", "mov", "wmv", "asf", "ts", "flv"].includes(ext)) {
                    continue;
                }
...
function append_files_to_list(path, files) {
    ...
    for (i in files) {
        ...
        if (item['mimeType'] == 'application/vnd.google-apps.folder') {
            ...
            html += ` <a class="countitems size_items list-group-item-action" style="text-decoration: none; color: ${UI.css_a_tag_color};" href="${pn}">${item.name}</a>${UI.display_download ? `<a href="${UI.show_credentials_in_url ? clear_pn += '?' + item.username + ':' + item.password : clear_pn}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></a>` : ``}${UI.display_size ? `<span class="badge bg-primary float-end"> ` + item['size'] + ` </span>` : ``}${UI.display_time ? ` <span class="badge bg-info float-end"> ` + item['modifiedTime'] + ` </span>` : ``}</div>`;
        }
    }
    if (is_file && UI.allow_selecting_files) {
        document.getElementById('select_items').style.display = 'block';
        ...
        fetch("/0:fallback", {
            ...
            .then(function(obj) {
                ...
                if (mimeType === "application/vnd.google-apps.folder") {
                    ...
                    //const url = UI.second_domain_for_dl ? UI.downloaddomain + obj.link : window.location.origin + obj.link;//USELESS
                    const url = UI.second_domain_for_dl ? UI.downloaddomain + path : window.location.origin + path;
                    const file_id = obj.id;
                    const decrypted_file_id = obj.decrypted_file_id;
                    if (mimeType.includes("video") || video.includes(fileExtension)) {
                        ...
                        file_video(name, encoded_name, size, poster, url, mimeType, file_id, cookie_folder_id, decrypted_file_id);
    ...
    fetch("", {
        ...
        .then(function(obj) {
            ...
            if (mimeType === "application/vnd.google-apps.folder") {
                ...
                const size = formatFileSize(obj.size);
                //const url = UI.second_domain_for_dl ? UI.downloaddomain + obj.link : window.location.origin + obj.link;//USELESS
                let url = UI.second_domain_for_dl ? UI.downloaddomain + path : window.location.origin + path;
                const file_id = obj.id;
                const decrypted_file_id = obj.decrypted_file_id;
                UI.show_credentials_in_url ? url += '?' + obj.username + ':' + obj.password : ''
                if (mimeType.includes("video") || video.includes(fileExtension)) {
                    const poster = obj.thumbnailLink ? obj.thumbnailLink.replace("s220", "s0") : UI.poster;
                    file_video(name, encoded_name, size, poster, url, mimeType, file_id, cookie_folder_id, decrypted_file_id);
                } else if (mimeType.includes("audio") || audio.includes(fileExtension)) {
      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ${navigation}
            ...
            <div class="card-body">
            <div class="input-group mb-4"${UI.hide_full_url ? ` style="display: none;"` : ``}>
              <input type="text" class="form-control" id="dlurl" value="${url}" readonly>
            </div>
            <div c
                        <div class="card-text text-center">
            ${UI.display_drive_link ? '<a type="button" class="btn btn-info" href="https://drive.google.com/file/d/'+ decrypted_file_id +'/view" id ="file_drive_link" target="_blank">GD Link</a>': ''}lass="card-text text-center">
            <div class="btn-group text-center">
        ...
        `<div class="card-body">
          <div class="input-group mb-4"${UI.hide_full_url ? ` style="display: none;"` : ``}>
            <input type="text" class="form-control" id="dlurl" value="${url}" readonly>
                      ...
                      <div class="card-text text-center">
            ${UI.display_drive_link ? '<a type="button" class="btn btn-info" href="https://drive.google.com/file/d/'+ decrypted_file_id +'/view" id ="file_drive_link" target="_blank">GD Link</a>': ''}
          <div class="card-text text-center">
            <div class="btn-group text-center">
              <a href="${url}" type="button" class="btn btn-primary">Pobierz</a>
              <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
...
function file_video(name, encoded_name, size, poster, url, mimeType, file_id, cookie_folder_id, decrypted_file_id) {
    //console.log(decrypted_file_id)
    var url_base64 = btoa(url);
    // Split the file path into parts
    const decoded_path = decodeURIComponent(window.location.pathname);
    //var path = window.location.pathname;//USLESS
    var pathParts = decoded_path.split('/');
    // Generate the navigation based on path parts
    var navigation = '';
    var new_path = '';
    for (var i = 0; i < pathParts.length; i++) {
        ...
        //kacperhaha's idea
        if (UI.change_number_of_disc_to_name && part === `${window.current_drive_order}:`) {
            part = window.drive_names[window.current_drive_order || 0];
        }
        navigation += '<a href="' + new_path + '" class="breadcrumb-item">' + part + '</a>';
    }
    let player = ''
    ...
    if (!UI.disable_player) {
        if (player_config.player == "plyr") {
            player = `<video id="playerr player" playsinline controls data-poster="${poster}">
      ...
      <source src="${url}" type="video/webm" />
            ...
            player = `<video id="playerr vplayer" poster="${poster}" muted=true class="video-js vjs-default-skin" controls preload="auto" width="100%" height="100%" data-setup='{"fluid": true}' style="--plyr-captions-text-color: #ffffff;--plyr-captions-background: #000000;">
      <source src="${url}" type="video/mp4" />
      <source src="${url}" type="video/webm" />
      <source src="${url}" type="video/avi" />
            ...
            player = `<div id="playerr player-container"></div>`
            ...
            player = `<div id="playerr player"></div>`
    ...
    <div class="container text-center"><br>
      ...
      <div class="card text-center">
        <div class="text-center">
          <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${name}<br>${size}</div>
          <div id="playersandbuttons">
${UI.disable_player || UI.disable_gdrive_player ? '<style>.player-buttons {display:none !important;}</style>' : ''}

<div class="player-buttons" style="display: flex;  width: 100%;  margin: 0;  padding: 0; border-radius: 10px 10px 0px 0px !important; overflow: hidden;">
<button class="btn btn-primary" id="gdriveButton" onclick="showGdrive()" style="flex: 1;   text-align: center;  font-size: 16px; height: 40px; border-radius: unset; box-shadow: initial;">Odtwarzacz GDrive</button>
<button class="btn btn-primary" id="PlayerButton"   onclick="showPlayer()"   style="flex: 1;   text-align: center;  font-size: 16px; height: 40px; border-radius: unset; box-shadow: initial;">Odtwarzacz ${player_config.player}</button>
</div>
<div id="players">
${player}
  <div id="gdrivePlayer" style="position: relative; padding-bottom: 56.25%; background-color: #30115e;">
${UI.disable_gdrive_player ? '<style>#gdriveButton {display:none;}</style>' : '<iframe id="GDriveiframe" src="https://drive.google.com/file/d/' + decrypted_file_id + '/preview" style="top: 0; left: 0; width: 100%; height: 100%; position: absolute; border: 0; opacity: 0; transition: opacity 1.5s ease-in-out;" allowfullscreen scrolling="no" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share;"></iframe>'}
    ${UI.disable_gdrive_player ? '' : '<style>[id*="playerr"] {display:none;}</style>'}
</div>
</div>
          </div>
          </div>
        </br>
            ${UI.disable_rotate_when_fullscreen ? '' : `<script>
            function setOrientation(orientation, retries = 1, delay = 1000) {
                if (window.screen?.orientation?.lock) {
                    window.screen.orientation.lock(orientation)
                        .then(() => console.log(\`Orientacja ustawiona na \${orientation}\`))
                        .catch((error) => {
                            console.error('Nie udało się ustawić orientacji:', error);
                            if (retries > 0) {
                                console.log(\`Ponawianie próby ustawienia orientacji... Pozostało prób: \${retries}\`);
                                setTimeout(() => setOrientation(orientation, retries - 1, delay), delay);
                            } else {
                                console.warn('Nie udało się ustawić orientacji po kilku próbach.');
                            }
                        });
                } else {
                    console.warn('API do blokowania orientacji ekranu nie jest dostępne.');
                }
            }

            function updateOrientationBasedOnFullscreen() {
                const isFullscreen = document.fullscreenElement ||
                                     document.webkitFullscreenElement ||
                                     document.mozFullScreenElement ||
                                     document.msFullscreenElement;

                const orientation = isFullscreen ? 'landscape' : 'portrait-primary';
            ...
            function debounce(func, wait) {
                ...
                const debouncedUpdateOrientation = debounce(updateOrientationBasedOnFullscreen, 2);
            function checkAndDeactivateScript() {
                const isLandscape = window.screen.orientation.type.startsWith('landscape');
                if (isLandscape) {
                    //console.log('Strona jest w trybie poziomym. Skrypt zostanie dezaktywowany.');
                    } else {
                        document.addEventListener('fullscreenchange', debouncedUpdateOrientation);
                        document.addEventListener('webkitfullscreenchange', debouncedUpdateOrientation);
                        document.addEventListener('mozfullscreenchange', debouncedUpdateOrientation);
                        document.addEventListener('MSFullscreenChange', debouncedUpdateOrientation);
                }
            }

            checkAndDeactivateScript();
            </script>`}
                ${UI.request_episodes_list ? `<script>
const loadIframeAndAnalyze = () => {
    let currentUrl = window.location.href;
    let trimmedUrl = currentUrl.substring(0, currentUrl.lastIndexOf('/') + 1);

    let iframeToGetEpisodes = document.createElement('iframe');
    iframeToGetEpisodes.src = trimmedUrl;
    iframeToGetEpisodes.style.border = "none";
    iframeToGetEpisodes.style.width = "0px";
    iframeToGetEpisodes.style.height = "0px";
    document.body.appendChild(iframeToGetEpisodes);

    iframeToGetEpisodes.addEventListener('load', () => {
        const iframeToGetEpisodesDocument = iframeToGetEpisodes.contentDocument || iframeToGetEpisodes.contentWindow.document;
        const script = iframeToGetEpisodesDocument.createElement('script');
        script.textContent = \`
        const analyzeiframeToGetEpisodesContent = () => {
            const iframeToGetEpisodesDoc = document;
            const listDiv = iframeToGetEpisodesDoc.querySelector('div#list.list-group.text-break');
            if (listDiv) {
                const links = listDiv.querySelectorAll('a.list-group-item-action');
                const validLinks = [];
                const extensions = ['.mp4', '.webm', '.avi', '.mpg', '.mpeg', '.mkv', '.rm', '.rmvb', '.mov', '.wmv', '.asf', '.ts', '.flv'];

                links.forEach(link => {
                    const href = link.getAttribute('href');
                    if (href.includes('?a=view')) {
                        const fileExtension = href.split('/').pop().split('?')[0];
                        if (extensions.some(ext => fileExtension.endsWith(ext))) {
                            validLinks.push({
                                text: link.textContent,
                                url: new URL(href, '\${trimmedUrl}').href
                            });
                        }
                    }
                });

                const currentIndex = validLinks.findIndex(link => link.url === '\${currentUrl}');
                const previousUrl = currentIndex > 0 
                    ? validLinks[currentIndex - 1].url 
                    : null;
                const nextUrl = currentIndex < validLinks.length - 1 ? validLinks[currentIndex + 1].url : null;
                ...
                console.log('No list found in iframeToGetEpisodes');
            }
        };
        ...
        const checkAndExecute = () => {
            ...
            const observerCallback = (mutationsList) => {
                const alertDiv = document.querySelector('div.d-none');
                if (alertDiv) {
                    setInterval(() => {
                    ...
                    observer.disconnect();
                    analyzeiframeToGetEpisodesContent();
                }
            };
        ...
        checkAndExecute();
        \`;
        iframeToGetEpisodesDocument.head.appendChild(script);
    });

    window.addEventListener('message', (event) => {
        if (event.data.type === 'VALID_LINKS') {
            const validLinks = event.data.validLinks;
            const currentUrl = event.data.currentUrl;
            const previousUrl = event.data.previousUrl;
            const nextUrl = event.data.nextUrl;

            const episodesButtons = document.createElement('div');
            episodesButtons.style.display = "flex";
            episodesButtons.style.width = "100%";
            episodesButtons.style.margin = "0";
            episodesButtons.style.padding = "0";
            episodesButtons.style.border = "2px solid rgba(255, 255, 255, 0.3)";
            episodesButtons.style.overflow = "hidden";
            
            const previousEpisode = document.createElement('a');
            previousEpisode.style.flex = "1";
            previousEpisode.style.textAlign = "center";
            previousEpisode.style.backgroundColor = "rgb(37, 13, 73)";
            previousEpisode.style.fontSize = "16px";
            previousEpisode.style.padding = "5px 5px";
            previousEpisode.style.borderRadius = "0px";
            previousEpisode.style.boxShadow = "initial";
            previousEpisode.style.height = "35px";
            previousEpisode.style.textDecoration = "none";
            previousEpisode.style.whiteSpace = "nowrap";        
            previousEpisode.style.overflow = "hidden";          
            previousEpisode.textContent = previousUrl ? "⏮️ " + validLinks.find(link => link.url === previousUrl)?.text : "Brak poprzedniego";
            previousEpisode.href = \`javascript:void(window.location.replace('\${previousUrl}'))\`;
            previousEpisode.style.display = previousUrl ? "" : "none";
            
            const nextEpisode = document.createElement('a');
            nextEpisode.style.flex = "1";
            nextEpisode.style.textAlign = "center";
            nextEpisode.style.backgroundColor = "rgb(37, 13, 73)";
            nextEpisode.style.fontSize = "16px";
            nextEpisode.style.padding = "5px 5px";
            nextEpisode.style.borderRadius = "0px";
            nextEpisode.style.boxShadow = "initial";
            nextEpisode.style.height = "35px";
            nextEpisode.style.textDecoration = "none";
            nextEpisode.style.whiteSpace = "nowrap";        
            nextEpisode.style.overflow = "hidden";          
            nextEpisode.textContent = nextUrl ? "⏭️ " + validLinks.find(link => link.url === nextUrl)?.text : "Brak następnego";
            nextEpisode.href = \`javascript:void(window.location.replace('\${nextUrl}'))\`;
            nextEpisode.style.display = nextUrl ? "" : "none";
            if(previousUrl && nextUrl)
            {
            previousEpisode.style.borderRight = "1px solid rgba(255, 255, 255, 0.3)";
            nextEpisode.style.borderLeft = "1px solid rgba(255, 255, 255, 0.3)";
            }
            ...
            select.style.backgroundColor = "#250d49";
            select.style.textAlign = "center";
            select.style.borderRadius = "0 0 10px 10px";
            select.style.border = "#FFFFFF4D 2px solid";
            select.style.textShadow = "0 0 1px rgba(255,255,255,.3),0 0 2px rgba(255,255,255,.3),0 0 5px rgba(255,255,255,.2)";
            ...
            const currentOption = Array.from(select.options).find(option => option.value === currentUrl);
            if (currentOption) {
            ...
            ${UI.show_next_and_previous_buttons ? `document.getElementById("playersandbuttons").appendChild(episodesButtons);
            episodesButtons.appendChild(previousEpisode);
            episodesButtons.appendChild(nextEpisode);` : ''}
            ${UI.show_select_episode_by_request ? `episodesButtons.style.borderBottom = "0px"; document.getElementById("playersandbuttons").appendChild(select);` : `episodesButtons.style.borderRadius = "0 0 10px 10px";`}
            ...
            document.body.removeChild(iframeToGetEpisodes);
        }
    });
};

loadIframeAndAnalyze();
</script>` : ''}

<script>
  function showPlayer() {
    document.querySelector('[id^="playerr"]').style.display = 'block';
    document.getElementById('gdrivePlayer').style.display = 'none';
  }

  function showGdrive() {
    document.querySelector('[id^="playerr"]').style.display = 'none';
    document.getElementById('gdrivePlayer').style.display = 'block';
  }

  const GDriveiframe = document.getElementById('GDriveiframe');
  if (GDriveiframe)
  {
  GDriveiframe.onload = function() {
    GDriveiframe.style.opacity = '1';
  };
  }
</script>
